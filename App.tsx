...
function App() {
  ...
  return (
    <div className="fixed inset-0 h-[100dvh] w-full flex flex-col bg-[#0f0518] text-white font-sans overflow-hidden">
      ...
      {/* HEADER */}
      <div className="bg-white/5 backdrop-blur-md z-20 px-4 py-3 pt-[max(0.75rem,env(safe-area-inset-top))] flex justify-center border-b border-white/5 relative shadow-lg shrink-0">
      ...
      {/* FLOATING ACTION BUTTONS - Centered constrained container */}
      <div className="absolute bottom-[calc(6rem+env(safe-area-inset-bottom))] w-full max-w-7xl mx-auto left-0 right-0 px-4 z-[50] flex justify-end pointer-events-none">
      ...
      {/* SEARCH HERE BUTTON */}
       {activeTab === 'map' && shouldShowSearchHere && !isRefreshing && (
          <div className="absolute bottom-[calc(6rem+env(safe-area-inset-bottom))] left-4 z-[50] pointer-events-auto animate-slide-up w-full max-w-7xl mx-auto px-4 flex justify-start pointer-events-none">
             <button
                onClick={handleSearchHere}
                disabled={isLoading}
                className={`w-12 h-12 rounded-full bg-slate-800/90 backdrop-blur border border-white/10 text-white flex items-center justify-center shadow-lg active:scale-95 transition-all pointer-events-auto ${isLoading ? 'opacity-50' : 'hover:bg-slate-700'}`}
             >
      ...
      {/* BOTTOM NAV - FLOATING ISLAND ON DESKTOP */}
      <div className="bg-black/40 backdrop-blur-xl border-t border-white/5 grid grid-cols-3 pb-[calc(1.25rem+env(safe-area-inset-bottom))] pt-3 z-[50] shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] w-full max-w-md mx-auto sm:mb-6 sm:rounded-2xl sm:border-x sm:border-t sm:border-white/10 relative sm:absolute sm:bottom-0 sm:left-0 sm:right-0">
        <button 
